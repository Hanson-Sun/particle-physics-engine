...
class Stiffjoint {
	constructor(list, c1, c2, len, breakforce, dampening) {
		this.c1 = c1;
		this.c2 = c2;
	...
	calc() {
		this.pos2 = this.c2.pos.add(this.c2.vel.mult(timestep));
		this.pos1 = this.c1.pos.add(this.c1.vel.mult(timestep));
		this.dp = this.c2.pos.sub(this.c1.pos);
		this.dpmag = this.dp.mag();
		this.dpdiff = (this.dpmag - this.len) * stiffness;
	...
	update() {
		...
		if (!this.c1.ispivot) {
			this.c1.vel = (this.c1.vel).add(this.dv.mult(0.5*this.dampening));
			this.c1.pos = (this.c1.pos).add(this.force.mult(timestep*timestep));

		} if (!this.c2.ispivot) {
			this.c2.vel = (this.c2.vel).sub(this.dv.mult(0.5*this.dampening));
...
p = new Vector2D(200, 200);

p3 = new Vector2D(200, 212);
v = new Vector2D(0, 0);
...
for (i = 1; i < 20; i++) {
	p2 = new Vector2D(200 + i * 12, 200);
	p4 = new Vector2D(200 + i * 12, 212);
